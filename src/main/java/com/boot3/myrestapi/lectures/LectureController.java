...
import com.boot3.myrestapi.common.excpetion.BusinessException;
...
public class LectureController {
	...
	public ResponseEntity getLecture(@PathVariable Integer id) {
		...
		// 한줄로 작성가능 Optional<T> orElsethrow(Supplier) Supplier의 추상 메서드 T get()
		// T extends Throwable
		...
		// 이렇게 한줄로 작성 가능
		LectureResource lectureResource = new LectureResource(modelMapper.map(lecture, LectureResDto.class));
	...
	public ResponseEntity updateLecture(
		...
		String errMsg = String.format("Id = %d Lecture Not Found", id);
		Lecture existingLecture = optionalLecture.orElseThrow(() -> new BusinessException(errMsg, HttpStatus.NOT_FOUND)
		);
		...
		LectureResource lectureResource = new LectureResource(modelMapper.map(savedLecture, LectureResDto.class));
		return ResponseEntity.ok(lectureResource);
	}
}
